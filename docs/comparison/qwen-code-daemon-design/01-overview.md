...
>
> 🎉 **Wave 1+2+2.5+3+4 + W5 PR 22a+22b/1 + 多 follow-up 同夜冲合**（2026-05-16~19 共 **35 MERGED + 8 OPEN/draft + 3 CLOSED**：22 Wave PRs + 7 follow-up + 3 bonus Stage 0 + 1 外部 + 2 refactor follow-up MERGED；1 PR 17 P2 follow-up + 1 telemetry polish + 3 adapter wire-up draft + **新 3 PR：W5 PR 22b/2 design + #4291 7-thread follow-up + E2E unbreak**；Wave plan 进度 **22.5/31 ≈ 73%**；**仅剩 Wave 5 PR 22b/2 收尾 + 22b/3 + 23/24/25/26 + Wave 6 共 ~10 PRs 待全开**）：
...
> - 🔧 Follow-up [PR#4302](https://github.com/QwenLM/qwen-code/pull/4302) telemetry Phase 1.5 polish — **OPEN** (doudouOUC, 2026-05-18 23:07, +255/-38 6 files；**follow-up to PR#4126 (Phase 1)，tracked in #4212**——独立 telemetry 工作流，**不在 daemon Wave plan**；session-tracing.ts fallback order + coreToolScheduler.ts exec span abort-as-result + log/span consistency)
...
> - 🔧 Wave 5 **PR 22b/2 design slice** [PR#4304](https://github.com/QwenLM/qwen-code/pull/4304) `BridgeOptions` lift + `DaemonStatusProvider` seam — **OPEN APPROVED** 2026-05-18 23:30 (doudouOUC, +852/-371 11 files；**3-stage 改 4-stage**：原 PR 22b/2 implementation lift 拆为 22b/2 design slice (冻 contract) + 22b/3 mechanical bulk lift (~3000 LOC, 1-2h IDE-driven `git mv`)；6 design decision baked in JSDoc：positional 参 / 全 envelope 返 / 无 abort/timeout / 单接口含 2 方法 / 可选 + idle fallback / acpChannelLive 由 bridge 传参；2 文件移：`BridgeOptions` interface (~150 LOC) → `acp-bridge/src/bridgeOptions.ts` + `buildDaemonPreflightCells` + `safeCheck` (~210 LOC byte-identical) → `cli/src/serve/daemonStatusProvider.ts`；新 factory `createDaemonStatusProvider()`；**Mode A in-process consumer 可省 `statusProvider?` 不崩** —— 对齐 PR 12/13 "idle status is queryable"；701/701 vitest pass)
