...
> - 🔧 Follow-up [PR#4245](https://github.com/QwenLM/qwen-code/pull/4245) integration test mirror align — **OPEN** (doudouOUC, 2026-05-17 14:13, +14/-4 2 文件；**release CI run 25992130532 跑挂触发**——PR#4237 + PR#4241 都改了 capabilities + EventBus 行为，但 integration test 只在 nightly/release pipeline 跑，PR CI 没抓到 drift；本 hotfix 校准：features list 18→24（漏 6 个）+ SSE backpressure 3→4 frames（slow_client_warning 强插）；**doudouOUC 自陈是该 mirror pattern 第三次 drift**，建议 follow-up 把 integration test 进 PR-CI lane 或 restructure assertion 去掉 inline expected list)
