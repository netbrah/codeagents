...
>
> 🎉 **Wave 1+2+2.5+3+4 全部完整收尾 + Wave 5 PR 22a 起手**（2026-05-16~18 共 **30 MERGED + 8 OPEN/draft + 2 CLOSED**：21 Wave PRs + 5 follow-up (含 PR#4279 Windows hotfix) + 3 bonus Stage 0 + 1 外部 MERGED；Wave 5 PR 22a OPEN + 3 post-merge follow-up (OAuth/init/mirror) + Wave 3 PR 14b + 3 adapter wire-up draft；Wave plan 进度 **21/31 = 68%**；**仅剩 Wave 5/6 共 10 PRs 待全开**）：
...
> - 🔧 Wave 5 **PR 22a 起手** [PR#4295](https://github.com/QwenLM/qwen-code/pull/4295) acp-bridge skeleton + zero-coupling primitives lift — **OPEN** (doudouOUC, 2026-05-18 16:27 **PR#4282 W4 收尾 0 分钟接力**；+1126/-732 17 文件；split PR 22 = 22a (zero-coupling lift) + 22b (deep refactor)；3 个 zero-coupling primitive 用 `git mv` 保 blame 移到新 `@qwen-code/acp-bridge` package：`eventBus.ts` (578 LOC 无 internal import) / `inMemoryChannel.ts` (73 LOC 只依赖 `@agentclientprotocol/sdk`) / `AcpChannel` types (`httpAcpBridge.ts:638-677`)；额外种 **`PermissionMediator` interface contract** 让 PR 24 4 strategies (first-responder / designated / consensus / local-only) 直接挂接；chiga0 #3803 "Stage 1.5-prereq AcpChannel lift" 真正开始；可独立 ahead of PR 17 + PR 14b ship 避 `httpAcpBridge.ts` merge-conflict)
> - 🔧 Follow-up [PR#4291](https://github.com/QwenLM/qwen-code/pull/4291) PR#4255 OAuth fold-in — **OPEN + CHANGES_REQUESTED** (doudouOUC, 2026-05-18 15:26, +418/-20 6 文件；**PR#4255 14:05 merged 20m 后 deepseek-v4-pro post-merge /review 抓 5 项**：① **[Critical]** `poll()` catch 写 raw err.message 到 stderr 镜像 `start()` ② **[Critical]** `runPollTick` 加 `DEVICE_FLOW_POLL_TIMEOUT_MS = 30_000` 防 non-abortable provider pin singleton 到 sweeper expiry 10min ③ `GET .../device-flow/:id` 只对 matching `initiatorClientId` echo 敏感字段 ④ `cancellerClientId` first-writer-wins ⑤ ...；PR#4255 史上最难合 (135 reviews / 20h39m) 之后**post-merge 还被抓 5 项**——review 难度极限示范)
> - 🔧 Follow-up [PR#4293](https://github.com/QwenLM/qwen-code/pull/4293) E2E baseline lockstep 修 — **OPEN + CONFLICTING** (doudouOUC, 2026-05-18 16:21, +3/-0 1 文件；**第八次 mirror drift**——PR 20/21 merged 后 `caps.features` baseline 漏 `workspace_file_bytes`/`workspace_file_write`/`auth_device_flow` 3 个 cap，**`main` E2E Tests run 26040351119 Linux sandbox:none/docker + macOS 全挂**；doudouOUC PR-internal lockstep 策略再次失败，daemon 项目 mirror pattern 工程债持续问题)
> - 🔧 Follow-up [PR#4297](https://github.com/QwenLM/qwen-code/pull/4297) PR#4282 post-merge P2 fixes — **OPEN** (doudouOUC, 2026-05-18 16:39, +345/-11 7 文件；**PR#4282 W4 PR 17 squash-merge 后 Codex /review 抓 4 P2 issue** 均现有测试套不可重现：① `POST /workspace/init` 用错文件名——daemon parent 不 `loadCliConfig` 致 `getCurrentGeminiMdFilename()` 停默认 `QWEN.md` 忽略 workspace `context.fileName: 'AGENTS.md'`，修法 `runQwenServe` 启动 snapshot value + forward via `BridgeOptions.contextFilename` ② `disabledTools` snapshot stale——`Config.disabledTools` `private readonly` frozen at construction，`setWorkspaceToolEnabled` 只更 settings.json，"toggle + restart" workflow re-register 刚 disable 的 tool，修法 restart MCP 用 fresh snapshot ③④...；daemon 项目工作流升级：**"review at merge"→"review continuously"** 标准动作 post-merge AI /review)
