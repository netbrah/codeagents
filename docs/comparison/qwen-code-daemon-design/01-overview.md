...
>
> ✅ **Wave 1 完整 + Wave 2 PR 5/6 ✅ + PR 7/8 双开**（2026-05-16~17 共 **8 MERGED** + Wave 2 PR 7/8 stack 推进，**无 block 点**）：
...
> - 🔧 Wave 2 **PR 8** [PR#4232](https://github.com/QwenLM/qwen-code/pull/4232) session-scoped permission route — **OPEN (draft, stacked on #4231)** (chiga0, 2026-05-17 07:04, +542/-5 11 文件, 1.15x 测试比, 5 test scenarios passing；新 route `POST /session/:id/permission/:requestId` + legacy 共存 + `permission_already_resolved` event + **bounded record 防 memory leak**（同 PR#4217 cap pattern 复用）+ 新 capability tag `session_permission_vote`；⚠️ 同 PR#4231 一并漏 integration test lockstep)
