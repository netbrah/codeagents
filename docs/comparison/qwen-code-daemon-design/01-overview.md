...
> - 🔧 Wave 2.5 **PR 9** [PR#4235](https://github.com/QwenLM/qwen-code/pull/4235) client heartbeat — **OPEN** (doudouOUC, 2026-05-17 08:57, +580/-2 14 文件, 测试 / 生产 ≈ 1.7x；`POST /session/:id/heartbeat` empty body → `200 { sessionId, clientId?, lastSeenAt }`；**4 安全不变式**：pre-validation BEFORE bump 防 random-id spam / `unregisterClient` drop per-client entry 防 stale 累积 / snapshot Map copy-on-read / anonymous heartbeat 只 bump session 水位；HTTP state route 推迟到 Wave 3 PR 12；feeds PR 12 diagnostics + Wave 5 PR 24 revocation policy；新 capability tag `client_heartbeat`；✅ 三套来源 lockstep 维持)
