...
**`qwen serve`** 是 Qwen Code 的 HTTP daemon 模式：一个长驻 headless 进程服务一个 workspace，把 AI 会话从"终端进程独占"变成"多客户端可同时接入的服务"。daemon 功能集已合入 main，随 v0.18.0 正式版发布（2026-06-12）。
...
- **已正式发布**：daemon 功能集经 `daemon_mode_b_main → main` 反向 merge 进入 main（#4490），随 **v0.18.0 正式版**发布（2026-06-12，#5050）
