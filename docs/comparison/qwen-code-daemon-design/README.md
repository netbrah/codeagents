...
- 🎉 **Wave 1+2+2.5+3+4 + W5 PR 22a+22b/1 + multi follow-up 同夜冲合**（2026-05-16~18 共 **35 MERGED + 5 OPEN/draft + 3 CLOSED**：22 Wave PRs + 7 follow-up + 3 bonus Stage 0 + 1 外部 + 2 refactor follow-up；PR 17 P2 fold-in + telemetry polish + 3 adapter draft；Wave plan 进度 **22.5/31 ≈ 73%**）：
  ...
  - 🔧 [PR#4302](https://github.com/QwenLM/qwen-code/pull/4302) Follow-up telemetry Phase 1.5 polish OPEN (doudouOUC, follow-up to PR#4126 tracked in #4212; **不在 daemon Wave plan**)
  - 🔧 [PR#4304](https://github.com/QwenLM/qwen-code/pull/4304) **Wave 5 PR 22b/2 design slice** `BridgeOptions` lift + `DaemonStatusProvider` seam **OPEN APPROVED** 2026-05-18 23:30 (doudouOUC, +852/-371 11 files; **3-stage 改 4-stage split** —— 原 22b/2 implementation lift 拆为 22b/2 design (冻 contract, 6 design decisions) + 22b/3 mechanical (~3000 LOC IDE-driven `git mv`); `statusProvider?` optional + idle fallback 让 Mode A in-process consumer 可省不崩)
