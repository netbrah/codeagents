...
**为什么选这个架构**：进程级隔离免费、crash 半径小、subagent isolation 自动成立、与 [PR#3889](https://github.com/QwenLM/qwen-code/pull/3889) child-process-per-session 实现一致。代价是 cold start ~1-3s/session、内存 ~30-50MB × N session——单机 N < 50 场景可接受。详见 [§18 单 vs 多 Session 设计深度对比](./18-single-vs-multi-session-design.md)。

**项目 scope**：qwen-code 只承诺 daemon building block（Stage 1 / 1.5 / 2，~3 周内 feature complete）。多 session orchestrator / 多租户 / 沙箱 / SaaS 部署等"平台层"作为 [External Reference Architecture](./08-roadmap.md#external-reference-architecture参考实现非项目路线图) 由外部实现，详见 [§18](./18-single-vs-multi-session-design.md) / [§19](./19-orchestrator-multi-tenancy.md) / [§11](./11-multi-tenancy-and-sandbox.md) /  设计参考。
...
| 🔒 **安全 / 多租户专题** | ~2 h | §11 → §19 → §07 → External SaaS HA → §15 | 企业部署评估 |
| 🌐 **远端 / 协作专题** | ~2 h | §12 → §14 → §15 | 客户端体验设计 |
| 💾 **数据架构专题** | ~1 h | §13 → §19 → External SaaS HA §三-§九 | 持久化 / HA 设计 |
...
| # | 文档 | 一句话 |
|---|---|---|
| 12 | [TUI 单进程 vs Daemon 兼容性](./12-tui-compatibility.md) | 4 层兼容性矩阵（显示层 100% / 状态层 100% / 数据源层替换 / 本地依赖 5 类 fallback）+ 多 TUI 共 session + 同 host fast path vs 跨 host RPC + 12 项兼容性测试 |
| 14 | [远端 CLI 模式与 Client Capability 协议](./14-remote-cli-mode.md) | 3 类拓扑（Local-Local / Local-Remote 不推荐 / **Remote-Remote 推荐**）+ Client Capability 反向 RPC 协议 + 5 类 capability + TLS/mTLS auth + NAT 穿透 + Local echo + VSCode Remote-SSH 对比 |
| 15 | [多端协调策略](./15-client-coordination.md) | 不限同类型 client 数量（保 collaboration 哲学）+ 6 类 client kind 分桶上限 + liveness（30s heartbeat / 90s 超时 / TCP RST 即时剔除）+ active typer 协调 + 显式 takeover + 可选 exclusive_per_type 模式 + IM bot 一对多用户 |
...
| # | 文档 | 一句话 |
|---|---|---|
| 13 | [实体模型与层级关系](./13-entity-model.md) | **5 层 hierarchy**（Tenant → Workspace → Session → Background Task → Tool Execution）+ 横切层（Client subscription）+ 认证侧 sidebar（External User / Token：不算 hierarchy）+ 关系矩阵 + 资源所有权层级表 + 生命周期表 + ER 图 |
...
| # | 文档 | 一句话 |
|---|---|---|
| 08 | [路线图](./08-roadmap.md) | qwen-code 主线 ~3 周内 feature complete：Stage 1（~1 周 ✅ Mode B headless PR#3889）/ Stage 1.5（~4d Mode A）/ Stage 2（~1-2 周 mDNS+OpenAPI+WebSocket+多 token+metrics）。**Orchestrator / 多租户 / 沙箱 / SaaS 部署作为 External Reference Architecture**（参考设计 §18 / §19 / §11 / External SaaS HA），由外部商业平台 / k8s operator 实现 |
| 09 | [与 OpenCode 详细对比](./09-comparison-with-opencode.md) | 路由 / 技术栈 / 设计哲学逐项对照 |
| 17 | [与 Anthropic Managed Agents 对比](./17-vs-anthropic-managed-agents.md) | **5 层架构对照**（client / agent runtime / tool / sandbox / persistence）+ **内置工具映射** + **协议层差异**（Anthropic 私有 vs ACP 标准）+ **双向 migration path**（Anthropic→Qwen / Qwen→Anthropic 兼容 API）+ **6 类客户场景推荐** + **决策树 6 问选型** + **3 种混合部署模式** + **"Managed Qwen Agents" 产品蓝图**（基于 External Reference Architecture 完整实施 (External Phase 1-4) 包装，6 月可建）|
| 18 | [单 vs 多 Session 设计深度对比](./18-single-vs-multi-session-design.md) | **22 维对比矩阵 + 6 项关键 tradeoff 深度分析**（隔离昂贵性 / cold start 平方根 / 内存 baseline 建模 / 隔离失败代价 / 复杂度守恒原理 / PR#3889 现实约束）+ **决策树 N≤5/50/100/500/500+** + 作为选型决策入口（不在主线扩展多 session 模型）|
...
| 多 client 并发请求 | [§03 决策](./03-architectural-decisions.md) §6 FIFO + fan-out + first responder + [§15 多端协调](./15-client-coordination.md) |
| 实体层级 | [§13 实体模型](./13-entity-model.md) 5 层 hierarchy + 认证侧 |
| 持久化 | [§19 持久化栈](./19-orchestrator-multi-tenancy.md)（持久层） JSON → SQLite → Postgres 演进 |
| 多租户 / 沙箱 | [§11 Shell 沙箱](./11-multi-tenancy-and-sandbox.md) + [§19 Orchestrator 多租户](./19-orchestrator-multi-tenancy.md) +  |
| HA / SaaS 部署 |  |
| 远端 CLI / 协作 | [§14 远端 CLI](./14-remote-cli-mode.md) + [§15 多端协调](./15-client-coordination.md) |
...
| 第三轮 | §11-§12 | 多租户 + 沙箱 + 越权防御 + TUI 兼容性 |
| 第四轮 | §13-External SaaS HA | 实体模型 + 持久层 + HA |
| 第五轮 | §14-§15 | 远端 CLI + 多端协调（client capability 协议）|
