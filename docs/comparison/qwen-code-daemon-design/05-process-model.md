...
>
> 本章下面"AsyncLocalStorage / LocalContext / Instance.directory" 等内容作为 "OpenCode multi-session 模型实现参考"保留（不在 qwen-code 主线）。
...
> **下面 §三-§四 章节描述的 `AsyncLocalStorage` / `Map<workspaceId, Instance>` / `LocalContext` 等内容**——是"如果未来扩展到 multi-session daemon 的实现参考"（不是当前 qwen-code 主线设计，仅作 OpenCode multi-session 模型对照）。读者如想对照 OpenCode multi-session 模型可参考；如只关注当前 1-session 模型可跳过。
...
const storage = new AsyncLocalStorage<InstanceContext>()  // 注：当前 1 daemon = 1 session 不需要 ALS Instance 路由层
