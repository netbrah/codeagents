...
| PR | 内容 | 状态 |
|---|---|:---:|
| **PR 22** `refactor(serve): extract acp bridge primitives` | `httpAcpBridge.ts` 拆为 shared `AcpChannel` + `Transport` + `EventBus` + bridge primitives；CLI route contract 保持（依赖 PR 4, 8, 11）| 🔧 **PR 22a OPEN [PR#4295](https://github.com/QwenLM/qwen-code/pull/4295)** (doudouOUC, 2026-05-18 16:27 **PR#4282 W4 收尾 0 分钟接力**；+1126/-732 17 文件；**split PR 22 = 22a (zero-coupling lift) + 22b (deep refactor)**——22a 可独立 ahead of PR 17/14b ship 避 `httpAcpBridge.ts` merge-conflict；**3 zero-coupling primitives 用 `git mv` 保 blame** 移到新 `@qwen-code/acp-bridge` package：① `eventBus.ts` (578 LOC 无 internal import) ② `inMemoryChannel.ts` (73 LOC 只依赖 `@agentclientprotocol/sdk`) ③ `AcpChannel`/`AcpChannelExitInfo`/`ChannelFactory` types (`httpAcpBridge.ts:638-677`)；**额外种 `PermissionMediator` interface contract**——PR 24 将实施 4 strategies (first-responder / designated / consensus / local-only) 直接挂接，替代 `BridgeClient.requestPermission` inline first-responder voting；chiga0 #3803 "Stage 1.5-prereq AcpChannel lift" 真正开始) |
