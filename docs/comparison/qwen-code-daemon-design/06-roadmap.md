...
| **2.5** Reliability + session lifecycle closure | client heartbeat + SSE replay sizing + slow-client warnings + session metadata/close-delete lifecycle | PR 9-11 — ✅ PR 9 [#4235](https://github.com/QwenLM/qwen-code/pull/4235) + PR 10 [#4237](https://github.com/QwenLM/qwen-code/pull/4237) MERGED 2026-05-17；🔧 PR 11 [#4240](https://github.com/QwenLM/qwen-code/pull/4240) OPEN+CONFLICTING | 1.5a #4 reliability + #2 lifecycle closure |
...
| **PR 11** session metadata + close/delete lifecycle | `DELETE /session/:id` force-close（即使其他 client 还 attached，cancel active prompt + resolve pending permissions as cancelled + emit `session_closed` event）+ `PATCH /session/:id/metadata` 更新 `displayName` (max 256 chars) + `GET /workspace/:id/sessions` 加 `createdAt`/`displayName`/`clientCount`/`hasActivePrompt`；新 `session_closed` (terminal) + `session_metadata_updated` events；新 capability tags `session_close` + `session_metadata`；双 close 幂等（SDK 把 404 翻译为 idempotent success）（依赖 PR 6, 7）| 🔧 **OPEN + CONFLICTING [PR#4240](https://github.com/QwenLM/qwen-code/pull/4240)** (doudouOUC, 2026-05-17 10:26 headRef `worktree-delightful-cooking-goose`——Claude Code worktree 生成 fingerprint；+1031/-39 15 文件 321 tests passing；✅ 三套来源 lockstep 维持 +68/-6 在 integration-tests；CONFLICTING 因 main 推过 PR#4235 后未 rebase) |
