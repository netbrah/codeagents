# 15 — 多端协调策略：subscriber 协议 / liveness / takeover

> [← 上一篇：远端 CLI 模式](./14-remote-cli-mode.md) · [回到 README](./README.md)
...
- ❌ 全局 hard limit "1 CLI per session"（[§14 §一](./14-remote-cli-mode.md) 列出 7 个被破坏场景）
...
interface Subscriber {
  ...
  capabilities: ClientCapabilities           // 见 §14 §3.5
...
每个 kind 的 token scope 限制（[§19 §四 AuthZ](./19-orchestrator-multi-tenancy.md#四authentication--authorization)）：
...
每次 takeover 写入 [§19 §六 audit log](./19-orchestrator-multi-tenancy.md#六audit-log)：
...
| §12 多 TUI 共 session | TUI 内部要实现本章的 typing focus / subscriber 列表 UI |
| External SaaS HA failover | activeTyper 状态需 in-memory，failover 后重协商 |
| §14 远端 CLI 模式 + capability | 本章 subscriber 元信息是 capability 路由依据 |
...
[← 返回 README](./README.md) · [下一篇：长跑稳定性 →](./16-stability-and-longevity.md)
