...
| 跨 daemon 重连 | 不支持——sessionId 绑定 daemon process；orchestrator 路由保证 client 连回原 daemon |
...
不同 daemon process 之间互相不可见——跨 daemon / 跨设备续行属于 **orchestrator 层**职责（`coordinator.sessionScope: 'single' / 'user' / 'thread'` 决定如何把 sessionId 路由到对应 daemon process）。详见 [§02 §1](./02-architectural-decisions.md#1-session-是否跨-client-共享) + [§14 Orchestrator 多租户与配额](./14-orchestrator-multi-tenancy.md)。

主要场景：
- **同 user 跨设备**（手机 → 电脑）：orchestrator `scope: 'user'` 路由到同一 daemon process（按 userId）
...
| `GET /coordinator/sessions` | 列出所有 active daemon processes | 必需 |
