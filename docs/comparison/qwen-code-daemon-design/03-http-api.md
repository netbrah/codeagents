...
- `sessionId` 必须存在于 `byWorkspaceChannel` 内某 channel 的 `sessionIds` set；不匹配 `404 session_not_found`
...
| `session_died` | channel.exited cleanup → 该 workspace 全部 session 死亡 |
