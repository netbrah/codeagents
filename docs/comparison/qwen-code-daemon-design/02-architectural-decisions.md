...
**PR#3889 Stage 1**：daemon instance 启动时绑定唯一 session，多 client 接入同一 daemon URL 即自动共享该 daemon 的这一 session（**Stage 1 框架下 Daemon Instance ↔ Session 是 1:1 关系**）。session 由 orchestrator 创建/分配 daemon instance 时确定。

> **Stage 2 in-process 演进路径**：复用 `QwenAgent.sessions: Map<sessionId, Session>` 已有多 session 能力后，"跨 client 共享同一 session"语义不变——只是底层从"多 client 接同一 daemon URL 自动落到该 daemon 唯一 session"变为"多 client 接同一 daemon URL + 同一 sessionId path 落到该 session"。共享 / fan-out / live collaboration 模型在两个阶段都成立。

| 维度 | 行为（Stage 1）|
|---|---|
| 1 Daemon Instance = 1 Session | HttpAcpBridge spawn-per-session 实现下进程级隔离，无 daemon 内多 session 路由 |
...
### Stage 1 选择 child-process-per-session 的依据

1. **进程级隔离免费**——一个 session crash 不影响其他（V8 / OS 自动）
2. **避开 Stage 1 引入跨 session 隔离的复杂度**——不需要 AsyncLocalStorage Instance ctx / per-session resource managers / Effect-TS LocalContext
3. **多租户简化**——daemon 不感知 tenant，orchestrator 层做 ACL
4. **资源生命周期清晰**——kill daemon = 清理所有 fd / child / memory（无需 per-session cleanup hooks）
5. **与 PR#3889 Stage 1 spawn-per-session 实现一致**——~0 改造成本快速 GA-ready

### 代价权衡

| 维度 | PR#3889 Stage 1 child-process-per-session | Stage 2 in-process N-session（与 OpenCode 同构）|
|---|---|---|
| 跨 session 资源共享 | ✗ 每 daemon 自己一份 LSP/MCP/cache | ✓ 共享省内存 |
| 隔离强度 | OS 进程级 | 应用层 ALS |
...
| 实现现状 | PR#3889 Stage 1 OPEN | `qwen --acp` agent 已具备（`QwenAgent.sessions: Map`），HTTP 路径留待 Stage 2 重构后接入 |

适用边界：Stage 1 单机 N < 50 经济性可接受；N ≥ 100 时投资源池化或推进 Stage 2 in-process 重构（详见 [§13 设计对比](./13-single-vs-multi-session-design.md)）。
...
**per-daemon MCP state**——每个 daemon instance 持有自己的一套 MCP client 集，daemon 退出全部清理。**不跨 daemon 实例共享**。Stage 1 child-process-per-session 模型下 daemon 进程边界天然就是 MCP children 的生命周期边界；Stage 2 in-process N-session 下范围将从 per-daemon 退化为 per-workspace（多 session 共 workspace 时复用同套 MCP），届时按 OpenCode `Effect.acquireUseRelease` 模式重构。
...
**Session 内私有**。不跨 session 共享。Stage 1 child-process-per-session 模型下 per-session = per-daemon；Stage 2 in-process N-session 下保持 per-session 严格私有（不向 daemon 内其他 session 泄漏），符合 PR#3717 既有语义。
...
| **Session state** | **per-session**（Stage 1 = per-daemon；Stage 2 in-process 下严格 per-session 不向其他 session 泄漏）| SessionService 持久化 + transcript JSONL |
| **FileReadCache** | **per-session**（Stage 1 = per-daemon；Stage 2 同样 session-scoped 不共享）| PR#3717 天然 session-scoped |
...
两种模式都遵循 Stage 1 "1 Daemon Instance = 1 Session" HttpAcpBridge 语义（决策 §2）——区别仅在于 daemon instance 是否同时承载本地 TUI 客户端。**Wire 协议字节级一致**——TUI（Mode A）走 in-process EventBus 替代 SSE。Stage 2 in-process N-session 重构后两种部署模式同步演进。
...
5. **与决策 §2 完全自洽**——两种模式都是 Stage 1 "1 daemon = 1 session" HttpAcpBridge 框架
...
### Stage 2 in-process N-session 下 Mode A 的 TUI 语义（关键设计澄清）

> **触发问题**（2026-05-12 维护者讨论）：ACP 多 session 时 TUI 是怎么处理的？
...
**Stage 2 in-process N-session 重构后 Mode A 行为选择**（2 个候选）：

| 选择 | 行为 | 优劣 |
|---|---|---|
| **A: Mode A 内 TUI 锁定 single-session，HTTP path 仍支持多 session** | TUI 启动时绑某 sessionId；其他 session 仅 HTTP 远程 client 可见——本地用户从 TUI 看不到 daemon 内并行 session | ✅ TUI UX 0 改动 / ❌ 异质（本地 TUI 与远程 client 视角不一致） |
| **B: Mode A 完全禁用多 session**（推荐）| `qwen --serve` 仍是 1 daemon = 1 session 框架；要多 session 必须 `qwen serve` headless | ✅ Mode A 语义简单 / ✅ TUI 不需要新 UI / ❌ 同一台机器要多 session 必须切 Mode B |

**推荐 B**：理由——① TUI 不存在多 session UI（要重做相当于新建一个 dialog 系统）；② 单机本地用户跑多 session 罕见（典型场景 N=1）；③ 简化语义。如果将来 TUI 真要 multi-session，应该单独立项重做 TUI 而不是从 daemon 模式倒推。

> **影响**：[§13 §4.3 决策树](./13-single-vs-multi-session-design.md#43-决策树)末枝 "推进 Stage 2 in-process N-session" 在 Mode A 路径下不成立——Mode A 永远 single-session；推进 Stage 2 隐含切到 Mode B headless。
...
| # | 决策 | 选择 | 关键依据 PR / 工具 |
|---|---|---|---|
| 1 | session 跨 client 共享 | **默认共享同一 daemon instance**（Stage 1）；scope 在 orchestrator 层（`coordinator.sessionScope`）| PR#3739 transcript-first fork resume + orchestrator 路由策略（§14）|
| 2 | 状态进程模型 | **PR#3889 Stage 1 = 1 Daemon Instance = 1 Session**（HttpAcpBridge spawn-per-session）；Stage 2 in-process 可复用 `QwenAgent.sessions: Map` 已有多 session 能力 | OS process 隔离 |
| 3 | MCP server 生命周期 | **Stage 1 per-daemon** + in-flight coalesce + 30s 健康检查（Stage 2 退化为 per-workspace 多 session 共享）| PR#3818 + 30s 健康检查（OpenCode 无）|
| 4 | FileReadCache 共享 | **per-session**（Stage 1 = per-daemon；Stage 2 in-process 仍严格 per-session 不向其他 session 泄漏）+ PR#3774 prior-read 守卫 + PR#3810 5 路径 invalidation | PR#3717 / PR#3774 / PR#3810 |
