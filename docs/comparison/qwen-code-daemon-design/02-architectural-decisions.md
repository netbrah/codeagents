...
> **术语演进**：早期 PR#3889 设计的 "Daemon Instance" 在 commit `6a170ef8`（2026-05-12）channel-per-workspace 重构后语义变了。本系列**统一使用以下 3 层术语**，避免歧义：
...
| **Channel**（= `qwen --acp` child）| 1 个 `qwen --acp` 子进程，绑定 1 个 workspace；持有 `QwenAgent.sessions: Map<sessionId, Session>` 内 N 个 session | `packages/cli/src/acp-integration/acpAgent.ts:194` |
| **Session** | ACP `Session` 实例（per-channel `sessions: Map` 内的一条记录）；持 transcript / FileReadCache / PermissionManager | `packages/cli/src/acp-integration/session/Session.ts` |

**核心约束**：
- 1 daemon process **可有 M channels**（每 workspace 1 channel；通过 `byWorkspaceChannel: Map` 路由）
- 1 channel **必须严格 1 workspace**（`acpAgent.ts:600` `this.settings = loadSettings(cwd)` 是 instance-wide 字段，跨 workspace 会污染）
- 1 channel **可持 N sessions**（同 workspace 多路复用 via `QwenAgent.sessions: Map`）

**关系图**：

```
qwen serve (1 Daemon process)
├─ Express HTTP front + EventBus + byWorkspaceChannel: Map
└─ M Channels (1 per workspace, M ≥ 1):
   ├─ Channel-A (workspace = /work/repo-a)
   │  └─ QwenAgent.sessions: Map → {sess-1, sess-2, sess-3}   ← 同 workspace N session
   ├─ Channel-B (workspace = /work/repo-b)
   │  └─ QwenAgent.sessions: Map → {sess-4, sess-5}            ← 跨 workspace 必走独立 channel
   └─ Channel-C (workspace = /work/repo-c)
      └─ QwenAgent.sessions: Map → {sess-7}
```

**废弃术语**："Daemon Instance" — 早期 PR#3889 设计中等同 "1 channel = 1 session"，commit `6a170ef8` 后此等式被打破，**本系列不再使用 "Daemon Instance"**；历史引用见各章上下文。
...
**支持两种部署模式 + 共享同一 daemon process 抽象**（术语见 [§〇 术语表](#〇术语表commit-6a170ef8-之后)）：
