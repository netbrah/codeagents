...
| 所有 client 都断开 + 空闲 N 分钟 | daemon instance 进入 idle，可被 orchestrator 回收（[§14](./14-orchestrator-multi-tenancy.md)）|
...
适用边界：单机 N < 50 并发 session 经济性可接受；N ≥ 100 时考虑资源池化或迁移到多 session 模式（详见 [§13 设计对比](./13-single-vs-multi-session-design.md)）。
...
| **TUI in-process bus** | 复用 §09 BackgroundTaskViewContext / SessionContext shape，订阅 EventBus 而非 SSE | 无 |
...
| **mDNS 广播**（§10） | 可选 `--discoverable` flag | 可选配置 `discovery.mdns: true` |
