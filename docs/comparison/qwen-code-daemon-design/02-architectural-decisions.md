...
7 个核心架构决策（决策 §1-§7）。最关键的是 §2 状态进程模型（commit `6a170ef8` channel-per-workspace + N session multiplexed）—— 这是其他 6 个决策的基础。**终态决策矩阵见 [§九](#九决策矩阵汇总)**。
...
| **Channel** | `qwen --acp` child = 严格 1 workspace × N session |
| **Session** | per-channel `sessions: Map` 内一条 |
...
**Stage 1.5 must-have #1** 落地后支持 per-request `sessionScope: 'thread'` override —— P2 同 workspace 内显式新建 isolated session，多 session 多路复用同 `qwen --acp` child（共享 OAuth / FileReadCache / CLAUDE.md parse / MCP children）。
...
├─ Express HTTP front + byWorkspaceChannel: Map<workspace, ChannelInfo>
└─ M Channels (1 per workspace, M ≥ 1):
   ├─ Channel-A (workspace = A) ─ qwen --acp #1
   │  └─ QwenAgent.sessions: Map → {sess-1, sess-2, sess-3}
   ├─ Channel-B (workspace = B) ─ qwen --acp #2
   │  └─ QwenAgent.sessions: Map → {sess-4, sess-5}
   └─ Channel-C (workspace = C) ─ qwen --acp #3
...
- **同 channel session crash 半径**：`channel.exited` cleanup 触发该 workspace 全部 session 收到 `session_died`；其他 workspace 不受影响

### 为什么不允许跨 workspace 共 channel
...
`this.settings` 是 `QwenAgent` instance 级字段（非 per-session）。`newSession({cwd})` 时 `loadSettings(cwd)` 覆盖；跨 workspace 同 channel 会污染 settings / MCP / OAuth。这是 commit `6a170ef8` 选择 "channel per workspace"（不允许跨 workspace 复用 channel）的**根本原因**。

### 演进背景（简）

早期 PR#3889 设计：1 Daemon Instance = 1 Session = 1 `qwen --acp` child（spawn-per-session）。Review 过程中（LaZzyMan / tanzhenxin / 维护者反馈）发现 `QwenAgent.sessions: Map` 已原生支持单 child 多 session（`yiliang114` VSCode 插件早已生产用）。commit `6a170ef8`（2026-05-12）重构为 channel-per-workspace + N session multiplexed。

### 代价权衡

| 维度 | PR#3889 Stage 1（channel-per-workspace + N session multiplexed） | Stage 2e native in-process（去 child）| OpenCode（跨 workspace 共享）|
|---|---|---|---|
...
| Crash 半径 | 同 channel 全部 N session | 整 daemon | 整 daemon |
| Cold start（首 session/workspace）| ~1-3s | ~10ms | ~10ms |
| Cold start（同 ws 第 N session）| **<200ms**（attach existing channel）| ~10ms | ~10ms |
...
| `channel.exited` cleanup 所有 session | 该 channel 全部 session 收 `session_died` event |
...
- 每个 channel 内的 child 持自己的一套 MCP client 集，child 退出全部清理
- 同 workspace N session **共享** MCP children
- 跨 workspace 不同 child 各自独立 MCP children（OS 进程级隔离）

### 依据

1. **MCP 持 workspace-specific state**：`filesystem` MCP 限制目录 / `git` MCP 持 repo path / 企业 DB MCP 持 workspace 连接串——per-channel 边界天然清晰
2. **配置可能微小差异**：同 `github` MCP 不同 workspace 可能用不同 token
3. **OpenCode `Effect.acquireUseRelease`** 可借鉴—per-workspace 范围（与 channel-per-workspace 自然对齐）

### Qwen 独有优化

| 优化 | 价值 |
|---|---|
| **PR#3818 in-flight rediscovery coalesce** | 同 channel 并发 reconnect 合并为单一 in-flight restart |
| **30s 健康检查 + 自动重连** | OpenCode 没有；掉线后用户主动 connect |

### 重复 spawn 代价

同 user 同 workspace N session 共 1 套 `github` MCP children（commit `6a170ef8` 后的 key win）。跨 workspace 同 daemon = M workspaces × M sets of MCP children。单 MCP ~50-200MB；N < 50 workspaces 可接受。
...
| **LSP server** | per-channel（= per-workspace）| 同 workspace N session 共享；跨 workspace 进程级隔离 |
| **MCP server** | per-channel（= per-workspace）| 同上 + reconnect coalesce + 30s 健康检查 |
| Background shell / agent / monitor / dream | per-task / 调度面 per-channel | task ID + sessionId 关联 |
...
两种模式都用同一套 Stage 1 channel-per-workspace + N session multiplexed 架构；区别仅在 daemon process 是否同时承载本地 TUI 客户端。**Wire 协议字节级一致** —— TUI（Mode A）走 in-process EventBus 替代 SSE。
...
| 2 | 状态进程模型 | **Stage 1 = 1 daemon + M qwen --acp children（1 per workspace）+ N sessions multiplexed per workspace**（commit `6a170ef8`）| 跨 workspace OS 进程隔离 + 同 workspace 应用层 ACP `sessions: Map` |
| 3 | MCP server 生命周期 | **per-channel（= per-workspace）** + in-flight coalesce + 30s 健康检查 | PR#3818 + 30s 健康检查（OpenCode 无）|
