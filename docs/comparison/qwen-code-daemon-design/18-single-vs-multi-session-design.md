# 18 — 单 Session vs 多 Session 设计优缺点深度对比

> [下一篇：Orchestrator 多租户与配额 →](./19-orchestrator-multi-tenancy.md) · [回到 README](./README.md)
...
| 15 | **Long-run 稳定性**（[§16](./16-stability-and-longevity.md)） | ✅ 10 个泄漏点 vanish 5 个 | ❌ 全部需 TTL/quota/circuit breaker |
...
**多 session 模式的"single point of failure"是它最大的运营负担**——所有 9 项稳定性模式（[§16 §五](./16-stability-and-longevity.md)）都是为了缓解此问题：TTL / bounded / quota / circuit breaker / memory threshold restart / heap dump / liveness / native supervisor / worker isolation。
...
| [§16 长跑稳定性](./16-stability-and-longevity.md) | 多 session 的 9 稳定性模式负担 |
| [§17 vs Anthropic Managed Agents](./17-vs-anthropic-managed-agents.md) | Anthropic 的 per-session container 与 Qwen 单 session 模型架构相似 |
