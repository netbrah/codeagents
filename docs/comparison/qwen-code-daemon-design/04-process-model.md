...
> **核心约束**（[§02 §2](./02-architectural-decisions.md#2-状态进程模型) **PR#3889 Stage 1** 1 daemon = 1 session HttpAcpBridge 框架）：
>
...
>
> **Stage 2 in-process N-session 演进影响**：① 引入 Node 内建 `AsyncLocalStorage`（不引 Effect-TS）做 per-request session ctx；② daemon 内仍只绑定一个 workspace（多 session 共 workspace），`Map<workspaceId, Instance>` 仍不需要；③ `process.cwd()` 仍不变（沿用 Stage 1 启动时绑定的约束）；④ 子进程 spawn 显式传 cwd 不变。
...
## 一、整体进程拓扑（PR#3889 Stage 1：1 daemon = 1 session）
...
PR#3889 Stage 1 1 daemon = 1 session = 1 workspace 模型下，daemon 主进程的逻辑 cwd 在**启动时一次性确定**，运行时不变。Stage 2 in-process N-session 重构后此约束保留——同一 daemon 仍只绑一个 workspace，多 session 共 workspace。
...
**Stage 1**：由于 1 daemon = 1 session，**daemon 内不需要 AsyncLocalStorage / `Instance.directory` 等 per-request ctx 路由**——所有 core 代码读 `config.getCwd()` 时拿到的都是 daemon 启动时绑定的那一个。

**Stage 2 in-process N-session**：`config.getCwd()` 不变（同 daemon 1 workspace），但 `config.getSessionId()` 退化为单值不再够用 —— 需引入 Node 内建 `AsyncLocalStorage` per-request 传 sessionId，core 代码改为读 `als.getStore()?.sessionId ?? config.getSessionId()`（fallback 兼容 Stage 1 路径）。
...
## 八、Effect-TS / ALS Instance 路由层：Stage 1 不需要，Stage 2 需要轻量 ALS
...
**Stage 1**：Qwen daemon 1 daemon = 1 session 模型下**完全不需要这层基础设施**：daemon 启动时就绑定唯一 cwd / sessionId / workspaceId，所有 core 代码读 `config.getCwd()` / `config.getSessionId()` 拿到的就是绑定的那一个。这是 Stage 1 简化的核心。

**Stage 2 in-process N-session**：cwd / workspaceId 仍是单值（同 daemon 1 workspace），但 sessionId 退化为 Map → 需引入 **Node 内建 `AsyncLocalStorage`**（不引 Effect-TS）做 per-request session ctx 传播。改动范围：HTTP 路由 handler 入口 `als.run({ sessionId }, ...)`，core 代码 `config.getSessionId()` 包装为 `als.getStore()?.sessionId ?? config.getSessionId()` fallback。比 OpenCode Effect-TS 路径轻得多（不引入新依赖、不重写 control flow）。

## 九、与 OpenCode 工作目录处理对比

| 维度 | OpenCode（multi-session） | PR#3889 Stage 1（1 daemon = 1 session）| Stage 2 in-process N-session |
|---|---|---|---|
| daemon 改 `process.cwd()` | ❌ | ❌ | ❌ |
| 上下文传播机制 | `LocalContext` (Effect-TS + AsyncLocalStorage) | **不需要**——daemon 启动绑定 cwd | Node 内建 `AsyncLocalStorage`（不引 Effect-TS）传 sessionId；cwd 仍 daemon 启动绑定 |
| 客户端声明 cwd 方式 | query / header / process.cwd() | daemon 启动 ENV / orchestrator spawn 参数 | 同 Stage 1 |
| 子进程显式 cwd | ✓ | ✓ | ✓ |
