...
│   ├─ in-process subscriber（Mode A 本地 TUI = super-client；详 §02 §7）
...
- 单机多 session：commit `6a170ef8` 后 daemon 自身在 workspace 维度多路复用（同 workspace N session via `QwenAgent.sessions: Map`，跨 workspace 独立 `qwen --acp` child）。跨 daemon process / 跨机器多 session 才需要 External orchestrator（[§01 §3.2](./01-overview.md#32-多-session-场景external-orchestrator--多-daemon-instances) + [§03 §8.2](./03-http-api.md#82-orchestrator-层-apiexternal-reference-architecture)）
...
- 新 session 创建（同 workspace 多 session 由 daemon 内 `connection.newSession({cwd, mcpServers})` 多路复用现有 channel；跨 workspace 由 daemon 内 `getOrCreateChannel` spawn 新 `qwen --acp` child；跨 daemon process 才需 orchestrator）
