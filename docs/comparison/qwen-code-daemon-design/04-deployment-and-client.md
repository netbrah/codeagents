...
daemon 已合入 main（[PR#4490](https://github.com/QwenLM/qwen-code/pull/4490)，随 v0.18.0-preview 线发布），形态只有一种：**`qwen serve` headless 长驻进程**，模型是 **1 daemon = 1 workspace × N session**；多 workspace = 多 daemon 进程。
...
|---|---|---|---|---|
| web-shell | `packages/web-shell` | REST+SSE（经 SDK + webui）| 浏览器终端 UI，React 组件可嵌入任意应用；daemon 第一个真实 web 消费者 | #4380 |
| webui | `packages/webui` | —（共享层）| daemon UI 共享层：typed events → transcript blocks → React bindings（`daemon-react-sdk`）| #4328 / #4353 |
| 官方桌面 app | `packages/desktop` | ACP SDK（捆绑 qwen-code runtime）| Electron 桌面 app | #3778 |
...
> **免责声明**：本文基于 qwen-code main 分支源码（`packages/cli/src/serve`、`packages/web-shell`、`packages/webui`、`packages/sdk-typescript` 等）与公开 PR 记录整理，状态截至 2026-06-12。daemon 功能随 v0.18.0-preview 线持续演进，路由与包结构可能变化，以源码为准。
