...
## 零、最新动态（截至 2026-05-08，含 v0.15.9 release）

### TL;DR

到 2026-05-08（v0.15.9 release），Qwen Code 在 subagent display 上**几乎所有 P0/P1 改进已合并**：
...
- 🟡 **[PR#3969](https://github.com/QwenLM/qwen-code/pull/3969) Ctrl+B promote keybind**（OPEN，#3831 PR-3 of 3）—— foreground → background promote 的 UI 终结篇，PR#3894 已合并提供 core 集成，PR#3969 加 Ctrl+B 用户键绑（详见 [§六.12](#已落地-12foreground--background-promote-pr3894-pr3969)）
- 🟡 **[PR#3970](https://github.com/QwenLM/qwen-code/pull/3970) TaskBase envelope + foreground subagent persistence**（OPEN）—— task-registry-unification design 的 PR-1 of 2：引入 shared `TaskBase` envelope（agent/shell/monitor 任务类型 intersect），foreground subagent 也接入 JSONL transcript writer + meta sidecar；agent-task discriminator 从 `flavor: 'foreground' \| 'background'` 改为 `isBackgrounded: boolean`
- 🟡 **[PR#3933](https://github.com/QwenLM/qwen-code/pull/3933) monitor notifications routing for subagents**（OPEN）—— Monitor 通知路由到启动 monitor 的 owning subagent（含 foreground / background / fork / resumed），原本 subagent-owned monitors 用 shared parent notification callback 污染 parent context
...
| [PR#3969](https://github.com/QwenLM/qwen-code/pull/3969) | 🟡 OPEN (+200/-12) | **PR-3 of 3**：Ctrl+B 用户键绑——按 Ctrl+B 让正在跑的 foreground shell 命令转 background，agent 当前 turn 立即 unblock，子进程继续跑 |
