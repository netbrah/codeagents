...
## 零、最新动态（截至 2026-05-07，含 v0.15.7 release）

### TL;DR

到 2026-05-07（v0.15.7），Qwen Code 在 subagent display 上有了**重大架构收敛事件**：

- 🌟 **PR#3909 ✓ 2026-05-07 13:38** —— Qwen Code **直接 port Claude Code 的 `CoordinatorTaskPanel` 模式**：移除 inline `AgentExecutionDisplay`，加 **always-on `LiveAgentPanel`** 锚定在**输入框 footer 下方**，一行一个 agent，右列 pin elapsed + tokens。Visual conventions（`○` 点 / `▶` 分隔 / `name: desc (activity)` 格式）从 Claude `CoordinatorTaskPanel` **逐字 port**（"so the two products feel consistent for users coming from Claude Code"）
...
- **Subagent 隔离与稳定性**修复完成（PR#3735 context auto-compact + PR#3873 + PR#3887 Config 隔离）
...
### 已落地 9：Subagent 稳定性与隔离（PR#3735 + PR#3873 + PR#3887）✓ 2026-05-06 / 05-07

三个连续 PR 闭合 subagent 的根本性问题：上下文溢出 + Config 隔离 + 4 fork path 全覆盖。
