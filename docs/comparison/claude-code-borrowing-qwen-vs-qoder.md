...
**判读**：Claude Code 的**运行时深度能力**多数是 **Qwen 单方面在追**——Computer Use / Tool Search / 结构化输出 / 分层压缩 / Speculation Qoder 全无。但**中途注入是例外**：两家都借了（Qwen `drainQueue` / Qoder `user_steering` 注入，命名不同、能力等价）——可见这类"交互必备"的运行时能力 Qoder 也会补。
