...
> 自托管 + 完整 runtime + Code Agent 定位的开源平台横向对比，对标 [Anthropic Managed Agents](./qwen-code-daemon-design/20-vs-anthropic-managed-agents.md)（闭源）。
>
> **更新日期**：2026-05-07
>
...
| 多租户 | **✓ Stage 4-6 设计中**（[§11 multi-tenancy](./qwen-code-daemon-design/11-multi-tenancy-and-sandbox.md)）|
| Sandbox | ✓ 5 种（None / OS-user / namespace / container / remote）[§11 §五](./qwen-code-daemon-design/11-multi-tenancy-and-sandbox.md) |
...
| **多租户** | ✓ | ❌ | ❌ | ❌ | ⚠ channel-scoped | **✓ Stage 4+ 设计**（[§11](./qwen-code-daemon-design/11-multi-tenancy-and-sandbox.md)）|
| **Sandbox** | ✓ managed | **✓ Docker** | ✗ | ✗ | ✗ | **✓ 5 种**（[§11 §五](./qwen-code-daemon-design/11-multi-tenancy-and-sandbox.md)）|
| **Session 持久化** | ✓ | ✓ EventStream | ✓ JSONL+SQLite | ✓ | ✓ FTS5 长期 | ✓ JSONL→SQLite→Postgres（[§15](./qwen-code-daemon-design/15-persistence-and-storage.md)）|
| **多 LLM provider** | ❌ Claude only | ✓ | ✓ | ✓ | ✓ 200+ | ✓ |
| **MCP 集成** | ✓ | partial | **✓ 一线** | **✓ 原生** | ✓ | ✓（[§06 §1](./qwen-code-daemon-design/06-mcp-resources.md)）|
...
| **多 client 共 session** | ❌（每 SDK call 独立）| ❌ | ❌ | ❌ | ⚠ via IM channel | **✓ live collaboration 默认**（决策 [§1+§6](./qwen-code-daemon-design/03-architectural-decisions.md)）|
...
| 本表 | 相关 codeagents 文档 |
|---|---|
| OpenCode 详细架构分析 | [Qwen daemon §09 OpenCode 详细对比](./qwen-code-daemon-design/09-comparison-with-opencode.md)（详尽 200+ 行）|
| Anthropic Managed Agents 详细 | [Qwen daemon §20 vs Anthropic](./qwen-code-daemon-design/20-vs-anthropic-managed-agents.md) |
...
| 多 client 共 session（Qwen 独家）| [Qwen daemon §18 多端协调](./qwen-code-daemon-design/18-client-coordination.md) |
| 长跑稳定性（任何 daemon 都需要）| [Qwen daemon §19 长跑稳定性](./qwen-code-daemon-design/19-stability-and-longevity.md) |
| 5 种 sandbox 设计 | [Qwen daemon §11 §五 Shell 沙箱方案](./qwen-code-daemon-design/11-multi-tenancy-and-sandbox.md) |
...
**展望**：
- 短期（6 月）：[Qwen Code daemon Stage 6](./qwen-code-daemon-design/16-high-availability.md) 落地后，开源 Code Agent 多一个完整 SaaS-ready 选项
