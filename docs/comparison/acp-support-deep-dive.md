...
**Codex**：用 MCP（Model Context Protocol）替代——`codex-mcp` crate 让 Codex 自己**作为 MCP server** 暴露给其他 LLM agent 调用。**MCP 是工具调用协议（窄）**，**ACP 是会话协议（全）**。Codex 选 MCP 反映它把自己定位为"被其他 agent 调用的工具"，而不是"被 IDE 嵌入的会话"。
...
**为什么 OpenCode 没做这件事**？OpenCode 已经有自家 HTTP daemon（`opencode serve` 13525 行 OpenAPI），所以走 OpenAPI 路线直接绕过 ACP——ACP 在 OpenCode 是 IDE-only 形态。两家殊途同归到"远端可用"，但路径不同。
...
| **远端 IDE 场景**（daemon over network） | 🔴 | 🔴 | 🟢 **唯一**（httpAcpBridge） | 🟡 走 HTTP daemon 但 IDE 仍只能 stdio ACP |
...
| 远端 / web 化 ACP 体验 | **Qwen** | 唯一有 httpAcpBridge daemon |
