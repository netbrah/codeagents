...
Qoder 的 subagent（工具名 `task` / `delegate`）执行时通过 **`subagentProgress`** 事件驱动 TUI（bundle 字符串证据，见 [EVIDENCE](../tools/qoder-cli/EVIDENCE.md#subagent-视图subagentprogress--tasks-面板)）：

| 维度 | Qwen Code | Qoder CLI v1.0 |
|---|---|---|
| 进度事件 | LiveAgentPanel 锚定输入框 + pill+dialog（PR#3909/#3768）| `subagentProgress` 事件 `{agentName, state, recentActivity}` |
| 状态机 | running/completed/error… | `pending → queued → running / in_progress → completed / error` |
| 实时活动 | transcript inline + panel | `recentActivity[]`（type `thought`/`action`/`tool_call`/`tool_result`，运行中显示 `"Working..."`）|
| 渲染风格 | LiveAgentPanel 专用面板 | 树形盒绘 `├└│⎿`（Gemini/Claude 血脉）+ `elapsed`/`duration` 计时 |
| **常驻并行面板** | ✅ **LiveAgentPanel**（always-on，锚定 footer）+ InlineParallelAgentsDisplay（`/review` 9-agent 密集面板）| ❌ 无常驻面板——**内联树形 + `/tasks` 后台面板**两态（`parallel` 多指并行工具调用，非并行 agent 编队）|
| 后台任务面板 | `/tasks` 调度入口 + 4-kind background tasks | **`/tasks`**（别名 `bg`/`background`）`custom_dialog` 面板，统一跟踪后台 subagent + shell |
| agent 管理 | `/agents`（create/manage）+ Agent Team | `/agents`（`createAgentsCommand()`，create/list）|

**判读**：两者 subagent 进度可视化都继承 Gemini CLI 的树形内联风格，但 **Qwen 额外投入了常驻 LiveAgentPanel**（直接 port Claude `CoordinatorTaskPanel` 模式，多 agent 并行时锚定输入框下方实时刷新），Qoder 仍是**内联进度 + 按需 `/tasks` 面板**，没有 always-on 的并行 agent 编队视图。
