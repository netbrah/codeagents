...
- **macOS Seatbelt 沙箱 profile**（两家都从上游带出，矩阵略有差异，见 [九、关键分歧](#九关键分歧)）
...
两家的 subagent 都从 Gemini CLI 基座继承（Agent 工具 + Markdown/YAML agent 定义 + worktree 隔离），但在**模型/资源控制、多代理协作、展示**三处分化明显。以下为源码（Qwen v0.18.0）与 bundle 解码（Qoder v1.0.18）逐项对照。
...
| 内置 agent | `Explore` + `statusline-setup` | `Explore`（"Fast agent specialized for exploring codebases"）+ `general-purpose`；另有 `agent-creator`/`skill-creator` 引导 skill |

### 6.2 调用与 LLM 接口（Agent 工具参数）

| 参数 | Qwen Code | Qoder CLI v1.0 |
|---|---|---|
| 选择 agent | `subagent_type`（省略 = 隐式 fork）| `subagent_type`（默认 `general-purpose`）|
...
| 维度 | Qwen Code | Qoder CLI v1.0 |
|---|---|---|
| 模型指定 | **具体模型名**（`qwen3.5-plus` 等，BYOK 任意 provider）| **6 档抽象**：`inherit`/`lite`/`efficient`/`auto`/`performance`/`ultimate`（绑定自营网关 credit 计费）|
| 思考力度 | runConfig | **`effort`**：`low`/`medium`/`high`/`max`（per-agent）|
| 轮次上限 | `maxTurns` | `maxTurns` |
| 超时 | runConfig timeout | **`timeoutMins`**（per-agent，分钟）|

> Qoder 的 `model` 6 档 + `effort` + `timeoutMins` 是一套**面向 credit 计费的资源旋钮**——把"用多强的模型、想多久、跑多少轮"显式参数化；Qwen 走"具体模型名 + BYOK"路线，资源成本取决于用户接的 provider。

### 6.4 多代理协作（Qwen 独有的一层）

| 能力 | Qwen Code | Qoder CLI v1.0 |
|---|---|---|
| 单层 subagent | ✅ Agent 工具 | ✅ Task 工具（`task`/`delegate`）|
| 隐式 fork | ✅ **`FORK_AGENT`**——省略 `subagent_type` 即 fork，继承全对话上下文 + 共享 cache（fork worker 不能再 spawn）| ❌ 未见等价 |
| **Agent Team** | ✅（实验，opt-in）`team_create`/`team_delete` + `task_create`/`update`/`list` + **`send_message`**——teammate 间 mailbox/inbox/broadcast、leader 角色、共享任务板 `~/.qwen/tasks/{team}` | ❌ **无团队层**——仅单层 Task，subagent 间无通信 |
...
| **常驻并行面板** | ✅ **LiveAgentPanel**（always-on 锚定 footer）+ InlineParallelAgentsDisplay（`/review` 9-agent 密集面板）| ❌ **内联树形 + `/tasks` 后台面板**两态，无 always-on 并行编队视图 |
| 后台面板 | `/tasks` + 4-kind background tasks | `/tasks`（`bg`/`background`）`custom_dialog` 面板，统一跟踪后台 subagent + shell |

### 6.6 一句话判读

**底座同源**（Agent 工具 + YAML 定义 + worktree 隔离 + run_in_background + depth 追踪），分化在三处：① **资源模型**——Qoder 用 6 档模型 + effort + timeoutMins 把成本参数化（服务 credit 计费），Qwen 用具体模型名 + BYOK；② **协作深度**——Qwen 多一整层 **Agent Team**（teammate 消息 + 共享任务板 + 编排工具 cron/monitor/workflow），Qoder 止步单层 Task；③ **展示**——Qwen 有常驻 LiveAgentPanel 并行编队视图，Qoder 是内联树形 + 按需 `/tasks` 面板。要多代理编排选 Qwen，要按档位控成本 + 与 Claude Code agent 配置同构选 Qoder。
...
| subagent | ✅ `/agents` + **Agent Team**（team/send_message）+ 常驻 LiveAgentPanel | ✅ `/agents` + `task`/`delegate` 工具 + `subagentProgress` 内联树形 + `/tasks` 后台面板（无常驻并行面板）|
