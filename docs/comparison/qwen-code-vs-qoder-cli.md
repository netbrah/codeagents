...
>
> **延伸**：本文是对称的双向对比；面向两团队整合的逐子系统对照 + 整合建议见 [Qwen × Qoder 整合能力对照](./qwen-code-qoder-integration.md)。
>
...
| 工具延迟加载（Tool Search）| `tool_search`（模型可调用：按 query 取回延迟工具 schema 并注册）| ❌ 无 `tool_search`；仅 `deferred_tools_delta` 渲染（推送"Tools now available/no longer"）| Qwen 独有；Qoder 那个是可用性变化通知，非搜索加载（详 [5.2](#52-tool-search工具延迟加载)）|
...
### 5.2 Tool Search（工具延迟加载）

**结论：Qoder 不支持 Tool Search。** Tool Search 指**模型可调用**的"按需加载延迟工具 schema"机制——延迟工具开机只暴露名字，模型用一个搜索工具按 query 取回 function declaration 并注册，后续才可调用，从而压缩 system prompt token（Claude Code 的 ToolSearch、Copilot 的 `TOOL_SEARCH` 同理）。

| 维度 | Qwen Code | Qoder CLI v1.0 |
|---|---|---|
| 模型可调用的 search 工具 | ✅ **`tool_search`**（`tools/tool-search.ts`："Fetches function declarations for deferred tools and registers them…"）| ❌ 无（`tool_search`/`search_tools`/`load_tool` 工具名全 0；工具导出表无 ToolSearchTool）|
| 延迟工具基础设施 | ✅ 延迟工具开机 reminder 仅列名 + `tool_search` 取回 schema | ⚠️ 有 **`deferred_tools_delta`**（渲染"Tools now available / no longer available"）+ `deferred_tool_use` 状态字段 |
| 性质 | **模型驱动**的按需发现加载（省 token） | **推送通知**：工具可用性变化（MCP 连断 / skill 激活等）的 transcript 渲染，**模型不能主动搜索加载** |
| 血脉 | Qwen 自加（gemini-cli 上游无 `tool_search`）| `deferred_tools_delta` 为 Qoder 自有（Qwen 源码与 gemini-cli 上游均无）|

> **辨析**：Qoder 的 `deferred_tools_delta` 容易被误认为 Tool Search，实则是**两个不同机制**——它只是把"工具集动态变化"渲染进对话（被动通知），没有让模型按需 query+加载延迟 schema 的 `tool_search` 工具。所以 Qoder 的工具是**全量注册进 system prompt**，不像 Qwen 能靠 `tool_search` 把大量 MCP/外部工具 deferred 起来省 token。
