...
> 阿里云 Qwen 团队出品的开源 AI 编程代理（Apache-2.0），支持中文界面，基于 Gemini CLI 深度定制。模型调用通过阿里云 Coding Plan 订阅或自带 API key（BYOK）接入。
...
首次启动会引导你选择认证方式（也可随时运行 `/auth` 重新配置）：

```bash
qwen          # 首次启动 → 运行 /auth 选择认证方式
```
...
| 方式 | 说明 |
|------|------|
| **Qwen OAuth** | 浏览器登录 `qwen.ai` 账户。**免费层已于 2026-04-15 终止**，新请求会被拒绝，请改用下面两种方式。 |
| **阿里云 Coding Plan** | 固定月费订阅，配额更高、可选模型多（Qwen / GLM / Kimi / Minimax 等）。 |
| **BYOK API Key** | 自带 OpenAI / Anthropic / Gemini 兼容的 key（含 DeepSeek、OpenRouter、Fireworks 等）。 |

### 使用阿里云 Coding Plan

```bash
# 在 bailian.console.aliyun.com 订阅 Coding Plan，获取 sk-sp-xxx key
export BAILIAN_CODING_PLAN_API_KEY="sk-sp-xxx"
export OPENAI_BASE_URL="https://coding.dashscope.aliyuncs.com/v1"
qwen --model qwen3-coder-plus
# 或运行 /auth 选择「Alibaba Cloud Coding Plan」交互配置
```
...
软件本身 Apache-2.0 开源。模型调用支持阿里云 Coding Plan 月度订阅（配额高、可选模型多），或自带 OpenAI / Anthropic / Gemini 兼容 key（BYOK，含 DeepSeek、OpenRouter 等），按用量计费、随时切换。

> **注意：** Qwen OAuth 免费层已于 2026-04-15 终止，不再提供第一方免费额度。
...
| 提供商 | 端点 | 说明 |
|--------|------|------|
| **阿里云 Coding Plan** | coding.dashscope.aliyuncs.com | 月度订阅（Qwen/GLM/Kimi/Minimax） |
| **Qwen OAuth** | chat.qwen.ai | 免费层 2026-04-15 已停 |
...
- **`qwen-oauth` 不可覆盖**——内置的 OAuth 认证类型无法通过 modelProviders 自定义（其免费层已于 2026-04-15 终止）
...
无论用 Coding Plan 订阅还是 BYOK，都建议养成节省 token 的习惯：

```bash
/compact                    # 定期压缩，减少 token 消耗
/model qwen3.5-plus         # 切换到已订阅/已配置 key 的模型
/plan                       # 先规划再执行，减少试错
```
...
|------|-----------|-------------|-------------|
| **计费方式** | Coding Plan 订阅 / BYOK（OAuth 免费层 2026-04-15 已停） | 订阅 / API | 订阅 |
...
> Qwen OAuth 免费层已于 2026-04-15 终止；如需稳定额度可订阅阿里云 Coding Plan，或自带 API key 按用量计费。
