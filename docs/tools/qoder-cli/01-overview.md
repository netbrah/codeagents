...
- **自营模型网关**：`api.qoder.sh`（全球）/ `api.qoder.com.cn`（国内）聚合 8+ 模型（Claude、GPT、Gemini、GLM、Kimi、Minimax、DeepSeek），账号统一计费
- **Claude Code 迁移链**：`qoder migrate --from-claude` 一次性迁移 + CLI 参数面对齐 Claude Code（`--permission-mode`/`--output-style`/`--add-dir` 等）+ marketplace 对接 `claude-plugins-official`
- **plugins + marketplace**：插件安装/市场源管理（`/plugins` `/marketplace`）
...
## CLI 参数（24 个，v0.x `--help` 确认；v1.0 已换为 commander 60+ 参数，见 [EVIDENCE](./EVIDENCE.md)）
...
| **Quest 模式** | ✓（3 场景 × 3 环境） | ✗ |
| **Claude 兼容** | `migrate --from-claude` 迁移 + CLI 参数面对齐 | hooks 事件 Claude Code 命名 |
| **Arena 模式** | ✗ | ✓（多模型竞争） |
...
1. **Quest 模式**：Spec→确认→执行→PR 完整工作流
2. **Claude Code 迁移成本低**：`migrate --from-claude` + CLI 参数/hooks 事件命名对齐 + Claude 插件市场对接
3. **8+ 模型自动路由**：自营网关统一计费，国内/全球双端点
4. **AI 贡献统计**：`commit` 子命令记录 AI 代码占比
5. **多种执行环境**：本地 / `--worktree` / `--remote` 远程任务（含 `--teleport` 会话转移）
...
5. **无本地 daemon runtime**：远程形态依赖 Qoder 云端账号体系（对比 Qwen Code 的 `qwen serve` 本地多端 daemon）
6. **文档 404**：部分文档页面不可访问
