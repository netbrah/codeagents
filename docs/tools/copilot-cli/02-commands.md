...
> Copilot CLI 共有 **61 个交互命令**（v1.0.61，`help commands` 权威输出；v1.0.11 基线为 34）+ 8 个 CLI 子命令 + 65 个 CLI 参数、12 个核心工具（族）、21 个浏览器工具、48+ 个 GitHub 平台工具、**7 个内置代理**。本文分析其命令注册/工具分类/代理定义设计，为 Code Agent 开发者提供命令系统架构参考。
>
...
| 代理 | 7 个 | 用户显式调用 / 自动批判 / 后台固化 | 专用任务（review/explore/task/rubber-duck/research/rem-agent/general-purpose） |
...
|-------|--------|--------|--------|---------|
| **Copilot CLI** | 61（v1.0.61） | 67+ | 7 | MCP + Plugin + 自定义代理 + 内置 skills |
| **Claude Code** | ~86 | 42 | Coordinator/Swarm | Skill + Plugin |
| **Gemini CLI** | ~41 | ~23 | 无 | TOML + Skill |
| **Qwen Code** | 57（v0.18.0） | 41 | Arena + Agent Team | BundledSkillLoader |

---

## 一、斜杠命令（61 个，v1.0.61 `help commands` 权威分组）
...
| 类别 | 命令 | 数量 |
|------|------|------|
| **Agent Environment** | `/init` `/agent` `/skills` `/mcp` `/plugin` | 5 |
| **Agents / Subagents** | `/model`（`auto` 自动选模）`/delegate`（送 GitHub 云端开 PR）**`/fleet`**（并行 subagent 编队）`/autopilot` `/tasks` | 5 |
| **Code** | `/ide` `/diff` `/pr` `/review` `/lsp` `/terminal-setup` | 6 |
| **Permissions** | `/allow-all` `/add-dir` `/list-dirs` `/cwd` `/reset-allowed-tools` | 5 |
| **Session** | `/resume` `/rename` `/context` `/usage` `/session`（delete/id 子命令）`/compact` `/share`（md/HTML/gist）**`/remote`**（GitHub web/mobile 远程控制）`/copy` `/rewind`（时间线选择器 + diff 统计） | 10 |
| **Help** | `/help` `/changelog`（可 AI 摘要）`/feedback` `/theme` `/statusline`（`/footer`）`/update` `/version` `/experimental` **`/memory`**（跨会话记忆开关）`/clear` `/instructions` `/streamer-mode` | 13 |
| **Other** | `/ask`（旁问）**`/chronicle`**（会话史检索/洞察/cost-tips）`/env` `/exit` `/keep-alive` `/login` `/logout` `/new` `/plan` **`/research`**（深度研究）`/restart` `/search` `/settings`（交互式设置 UI）**`/sidekicks`** `/undo` `/user` **`/voice`**（Foundry Local 本地听写） | 17 |

### 1.2 功能标志门控命令（64 个 flag，含 rollout 状态）

v1.0.61 的 flag 体系已从早期 7 个本地开关进化为 **64 个带 rollout 阶段的 flag**（`off` → `staff` → `team` → `staff-or-experimental` → `experimental` → `on`），形态接近 Codex 的 feature lifecycle：

| 命令/能力 | 功能标志 | 状态 |
|------|---------|------|
| `/voice` 语音听写 | `VOICE` | **on** |
| rubber-duck 自动批判代理 | `RUBBER_DUCK_AGENT` | **on**（1.0.58 起默认启用）|
| 远程 JSON RPC | `REMOTE_JSON_RPC` | **on** |
| `/security-review` | `SECURITY_REVIEW` | experimental |
| `/worktree`（`/move`，带未提交改动切 worktree）| `WORKTREE` | experimental |
| Tool Search 延迟加载 | `TOOL_SEARCH` | experimental |
| `/every` `/after` 定时调度 | `EVERY_AND_AFTER` | staff-or-experimental |
| **shell 命令沙箱** | `SANDBOX` | staff-or-experimental |
| `/subconscious`（rem-agent 记忆固化）| `COPILOT_SUBCONSCIOUS` | team |
| computer use | `COMPUTER_USE` | staff |
| GitHub TUI（issues/PR/gists tabs + 主题）| `COPILOT_GITHUB_TABS/THEME` | staff-or-experimental |

**开发者参考**：与 Claude Code 的 22 个 build-time flag + GrowthBook 远程灰度、Codex 的 52 个生命周期 flag 相比，Copilot 的 64-flag 体系是三者中阶段最细的（6 档 rollout），且 flag 描述明文内嵌在 bundle 中可审计。
...
## 五、内置代理系统（7 个，YAML 定义）

> **v1.0.61 现状**：内置代理数组为 `["explore","task","code-review","rubber-duck","research","rem-agent","general-purpose"]`，`definitions/` 另含 `security-review.agent.yaml` 与 `sidekick/{github-context,subconscious-agent}.yaml`。本节 5.2~5.4 的 code-review / explore / task 详析仍有效；v1.0.12 之后新增的 4 个：
>
> | 代理 | 用途 | 状态 |
> |---|---|---|
> | **rubber-duck** | 用互补模型对当前工作做独立批判（GPT 会话由 Claude 批判）；`/rubber-duck` 显式调用，`builtInAgents.rubberDuckAutoInvoke` 可自动触发 | 1.0.58 起默认启用 |
> | **research** | `/research` 深度研究——GitHub search + web 多源调研，产出可 `/share` 的研究报告 | 正式 |
> | **rem-agent** | **跨会话记忆固化**——读取会话轨迹，用 `context_board` 工具增删 "dynamic context board"，让同仓库后续会话受益；由 `/subconscious run` 后台启动 | team 灰度 |
> | **general-purpose** | 通用 subagent（对标 Claude Code 同名代理）| 正式 |
>
> 另有 **Critic agent**（1.0.18，experimental）自动用互补模型审查 plan 与复杂实现。
...
## 七、CLI 参数系统（65 个参数，v1.0.61；v1.0.11 基线 57）
