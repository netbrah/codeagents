...
Copilot CLI 是 GitHub 推出的终端原生 AI 编程代理，其竞争壁垒不在于模型能力（它用的是 Claude、GPT、Gemini 等第三方模型），而在于**平台集成深度**——48 个 GitHub 平台工具、21 个浏览器工具、**7 个 YAML 定义的内置代理**（v1.0.61：explore / task / code-review / rubber-duck / research / rem-agent / general-purpose）、Claude（含 Fable 5 / Opus 4.8）+ GPT + Gemini 多模型阵容 + `auto` 自动路由 + BYOK 自定义 provider。这是唯一一个将 Actions、PR、Issues、代码扫描、密钥扫描全部原生联动的 CLI Agent，且 `/delegate` 可把会话直接送 GitHub 云端生成 PR。
...
| **内置代理** | **7 个 YAML 定义**（+rubber-duck 批判 / research 深研 / rem-agent 记忆固化 / general-purpose，v1.0.61） | Arena + Agent Team | 中 | YAML 声明式代理 + 互补模型批判（rubber-duck 用 Claude 批判 GPT 会话） |
| **模型选择** | Claude（Fable 5 / Opus 4.8 / 1M 档）+ GPT + Gemini + `auto` 路由 + **BYOK 自定义 provider** | 通义千问系列 + BYOK | 路线趋同 | `auto` 按会话自动选模 + 上下文档位端到端强制（200K vs 1M） |
| **命令系统** | **61 个交互命令**（v1.0.61；另有 /security-review /worktree /every 等 flag 门控） | 57 个（v0.18.0） | 接近 | 功能标志门控命令的模式可参考（64 个 flag 含 rollout 状态） |
